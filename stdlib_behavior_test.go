...
func TestStdlibPathCleaning(t *testing.T) {
	...
	if !isRedirect(w.Code) {
		t.Fatalf("got %d, want a redirect (stdlib canonicalization)", w.Code)
	}
...
func TestStdlibTrailingSlashRedirect(t *testing.T) {
	...
	if !isRedirect(w.Code) || w.Header().Get("Location") != "/articles/" {
		t.Fatalf("got %d %q, want a redirect -> /articles/", w.Code, w.Header().Get("Location"))
	}
